...
# Add best fitting model per participant
exp2_cum_score <- merge(exp2_cum_score, model_comp_2[, c("human_id", "Best_model")], by="human_id")

temp2 <- exp2_cum_score[,c("t","acc_sum","Best_model","condition","part_num")]
dat_by_model_2 <- temp2 %>% group_by(Best_model,t) %>% 
  summarize(model_acc_sum = mean(acc_sum))
...
ggplot(data = dat_by_model_2, aes(x = t, y=model_acc_sum, group = Best_model)) + 
   geom_line(aes(color= Best_model)) +
...
```{r exp2-cumScores-bis, fig.cap = "Average cumulative scores of participants in experiment 2 by best fitting model", fig.align='center'}
# 
