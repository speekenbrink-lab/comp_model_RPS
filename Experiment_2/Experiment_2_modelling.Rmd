...
TOM_model <- function(par, data, opp_strategy_vec, return_value, within_transfer, btwn_transfer, Naive = FALSE) {
  ...
  for(t in 1:nrow(data)) {
    ...
    if(data[t,"round"] == 1) {
      ...
      #Naive 
      if (Naive) {
        # pass. no update to prior vector throughout all rounds and games
      } else {
        #abc
        #def
        # Round 1 of stage 1 
...
TOM_model(c(0.8,0.1),test_data,c("level0","level1","level2"),"-2loglik", within_transfer = FALSE, btwn_transfer = FALSE, Naive = TRUE)
```
