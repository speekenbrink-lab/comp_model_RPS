...
```{r}
naive_bayes <- function(par,data,opp_strategy_vec,return_value,opp_mod_transfer=TRUE) {
...
for(id in unique(dat$human_id)) {
  ...
  tdat <- subset(dat,human_id == id)
# TR_bayes_modelling[[id]] <- DEoptim(fn=naive_bayes, lower = 0.0, upper = 1.0, data=tdat,opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", opp_mod_transfer = TRUE, control=list(trace = FALSE,parallelType=1))
...
All_results <- rbind(All_results,
                       data.frame(
                         ...
                         "QL_states_BIC" = QL_states_modelling[[id]]$value + 2*log(150),
                         # beta ->  inverse temperature parameter in softmax choice function
                         "QL_states_Beta" = QL_states_modelling[[id]]$par[1],
                         # alpha -> learning rate in QL update
                         "QL_states_alpha" = QL_states_modelling[[id]]$par[2],
...
```
# Looks like most participants behavior can be explained by Bayes with or without transfer. Most others are consistent with random behavior, and only 1 qith QL and 1 with S_EWA. 
...
```
# Notes : There is positive and stat significant correlation between the difference between the best model BIC and random BIC on one hand, and the early scores from the target games ( FWG and NUM). 
...
Bayes_only <- subset(Corr_Tr_model, model != "Random" )

Bayes.aov <- aov(Early_game_score ~ model, data = Bayes_only)
# Summary of the analysis
summary(Bayes.aov)
TukeyHSD(Bayes.aov)

wilcox.test(Early_game_score ~ model,data = Bayes_only) 


# Total time by best predictive model
tapply(Corr_Tr_model$Total_time, model, mean)
time.aov <- aov(Total_time ~ model, data = Corr_Tr_model)
# Summary of the analysis
summary(time.aov)
TukeyHSD(time.aov)

``` 
#Scores are indeed higher but difference is not statistically significant. 
