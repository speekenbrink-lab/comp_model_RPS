...
## Modelling fixed level-k type participant
```{r}
...
## Fixed level-k type: Likelihood of the different models by trial
...
# Fitting a bayesian model with NO TRANSFER of opp strat distribution between games 
```{r}
...
Corr_Tr_model$model <- recode(Corr_Tr_model$Best_model,"Bayes Tr" = "Tranfer","Bayes No Tr" = "No Transfer", "QL_states" = "Q_Learning",  .default = "Random")
model <- factor(Corr_Tr_model$model)
condition <- factor(Corr_Tr_model$condition)

# Total score by best predictive model 
tapply(Corr_Tr_model$Total_score, model, mean)

# Compute the analysis of variance
res.aov <- aov(Total_score ~ model + condition, data = Corr_Tr_model)
...
Non_Random_Only <- subset(Corr_Tr_model, model != "Random" )
