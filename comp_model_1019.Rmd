...
```{r}
naive_bayes <- function(par,data,opp_strategy_vec,return_value) {
  # par = parameter, noise in computer choice
  # dat = data subset for one participant
...
for(id in unique(dat$human_id)) {
  ...
  tdat <- subset(dat,human_id == id)
  naive_bayes_modelling[[id]] <- optim(c(0.1,0.1),fn=naive_bayes,gr = NULL, data=tdat,opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", lower = c(0.01,0.01), upper = c(0.99,0.99), method="L-BFGS-B")
}
...
Q_learn <- function(par,data,return_value=c("-2loglik","likelihood_by_trial")){
  ...
  for(t in 1:nrow(data)) {
    ...
    if(data[t,"round"] == 1) {
      ...
      Q_vals[h_act_prev] <- Q_vals[[h_act_prev]] + as.numeric(lambda)*( reward - Q_vals[[h_act_prev]])
...
for(id in unique(dat$human_id)) {
  ...
  tdat <- subset(dat,human_id == id)
  QL_modelling[[id]] <- optim(c(1,0.1),fn=Q_learn,gr = NULL, data=tdat,"-2loglik", lower = c(0.001,0.001), upper = c(10,0.99), method="L-BFGS-B")
}
...
lambdas <- seq(0,100,1)
data_test <- subset(data,human_id =="oXdDF_U-Dft2ja2lAAAs")

plot(lambdas,EWA_self(lambdas,data_test,"-2loglik"))
```
