...
#   tdat <- subset(dat,human_id == id)
#   naive_bayes_modelling[[id]] <- optim(c(0.1,0.1),fn=naive_bayes,gr = NULL, data=tdat,opp_strategy_vec = c("level0","level1","level2"),"-2loglik", lower = c(0.01,0.01), upper = c(0.99,0.99), method="L-BFGS-B")
# }
# 
...
#   tdat <- subset(dat,human_id == id)
#   QL_modelling[[id]] <- optim(c(0.1,0.1),fn=Q_learn,gr = NULL, data=tdat,"-2loglik", lower = c(0.001,0.01), upper = c(0.99,10), method="L-BFGS-B")
# }
# 
...
# 
# 
EWA_par <- function(par,data,return_value=c("-2loglik","likelihood_by_trial")){
...
for(id in unique(dat$human_id)) {
  ...
  EWA_modelling[[id]] <- DEoptim(fn=EWA_par,lower = c(0.001,0.001,0.001,0.001), upper = c(1,0.99,0.99,1), data=tdat,"-2loglik")
}
...
# }
  EWA_self_modelling[[id]] <- DEoptim(fn=EWA_self, lower = 0.0, upper = 10.0, data=tdat)
}
