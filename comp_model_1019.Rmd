...
```
##### Looks like most participants behavior can be explained by Bayes with or without transfer. Most others are consistent with random behavior, and only 1 with QL and 1 with S_EWA. 
...
### Model based model: compute transition probabilities to new states. Weigh the value of an action by the conditional probabilty of ending up in a future state.... once you take an action in current round, you can only end up in 3 states, with probabilities p, q and 1-p-q. You are learning these probabilities by updating priors (initial prior is dirichlet). 
```{r}
